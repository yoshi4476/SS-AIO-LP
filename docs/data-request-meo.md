...
`docs/meo-data-template.csv` を開いて、`SAMPLE` で始まる3行を消し、
実際の店舗データを1行1店舗で入れる。

**SAMPLE行を消し忘れても集計には入らない。** 記入例のまま実行すると
「実データがありません」で止まる。架空の数字が記事に出ることはない。

列が足りなくても動く。`store_id` `industry` `start_ym` `end_ym` の4つが
あれば集計でき、口コミや返信率の列は入っている分だけ集計に使う。
