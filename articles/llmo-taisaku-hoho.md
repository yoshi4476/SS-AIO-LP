...
当社は自社サイトをAIO対策の実験場にしており、構造化データ・llms.txt・主要AIクローラー20種の許可を実装したうえで引用状況を日次で計測しています。**実装から2週間の時点では、検索表示は102語・370回まで増えた一方、クリックは0回**でした（2026年8月時点・Search Console実測）。
