...
READ_AS = [
    ...
    (r"(?<![A-Za-z])Q&A(?![A-Za-z])", "キューアンドエー"),
    (r"llms\.txt", "エルエルエムエス テキスト"),
    (r"Overview", "オーバービュー"),
    (r"(?<![A-Za-z])UA値", "ユーエーち"),
    (r"(?<![A-Za-z])C値", "シーち"),
    (r"◯◯|○○", "まるまる"),
    (r"施主", "せしゅ"),
    (r"延床", "のべゆか"),
    # 読みが割れる語。音読み・訓読みを取り違えると意味が変わって聞こえる
    (r"(?<![0-9０-９])一行", "いちぎょう"),
    # 音声認識で聞き直して見つけた読み違い（2026-09 実測）
    (r"断熱等性能等級", "だんねつ とうせいのう とうきゅう"),
    (r"性能値", "せいのうち"),
    (r"上乗せ", "うわのせ"),
    (r"売主", "うりぬし"),
    (r"買主", "かいぬし"),
    ...
    (r"瑕疵", "かし"),
    (r"No\.\s?1", "ナンバーワン"),
    (r"ChatGPT", "チャットジーピーティー"),
    (r"Perplexity", "パープレキシティ"),
    (r"Gemini", "ジェミニ"),
    # 語を並べる「・」は、読まれずに語がつながって聞こえる
    (r"(?<=[^\s　])・(?=[^\s　])", "、"),
]

# 助数詞の読み。数字のまま渡すと「いちかげつ」「じゅうけん」「さんつ」と読まれる
COUNTERS = {
    "か月": {1: "いっかげつ", 2: "にかげつ", 3: "さんかげつ", 4: "よんかげつ", 5: "ごかげつ", 6: "ろっかげつ",
            7: "ななかげつ", 8: "はちかげつ", 9: "きゅうかげつ", 10: "じゅっかげつ", 12: "じゅうにかげつ"},
    "件": {1: "いっけん", 6: "ろっけん", 8: "はっけん", 10: "じゅっけん"},
    "つ": {1: "ひとつ", 2: "ふたつ", 3: "みっつ", 4: "よっつ", 5: "いつつ", 6: "むっつ", 7: "ななつ",
          8: "やっつ", 9: "ここのつ"},
...
def _man(m):
    """1万以上は「万」を入れて渡す。55393 は「ごまんごせん…」ではなく桁ごとに読まれることがある"""
    n = int(m.group(0))
    if n < 10000:
        return m.group(0)
    oku, rest = divmod(n, 100000000)
    man, low = divmod(rest, 10000)
    return (f"{oku}億" if oku else "") + (f"{man}万" if man else "") + (str(low) if low else "")


def kana_num(n):
    """1〜9999 をかなで読む。数字のまま渡すと「32坪」が「さんじゅうふたつぼ」と読まれた"""
    d1 = ["", "いち", "に", "さん", "よん", "ご", "ろく", "なな", "はち", "きゅう"]
    out = ""
    for v, unit, sp in ((1000, "せん", {1: "せん", 3: "さんぜん", 8: "はっせん"}),
                        (100, "ひゃく", {1: "ひゃく", 3: "さんびゃく", 6: "ろっぴゃく", 8: "はっぴゃく"}),
                        (10, "じゅう", {1: "じゅう"})):
        q, n = divmod(n, v)
        if q:
            out += sp.get(q, d1[q] + unit)
    return out + d1[n]
...
def _counter(m):
    ...
    if unit == "坪":
        return kana_num(n) + "つぼ" if 0 < n < 10000 else m.group(0)
    key = "か月" if unit in ("ヶ月", "カ月") else unit
    return COUNTERS.get(key, {}).get(n, m.group(0))


def _paren(m):
    """括弧の中身: 英数字だけ（論文番号など）は読まない。それ以外は間を取って読む"""
    inner = m.group(1) if m.group(1) is not None else m.group(2)
    if re.fullmatch(r"[\x00-\x7f]+", inner):
        return ""
    return "、" + inner.rstrip("。") + "、"
...
def read_text(s):
    ...
    for pat, rep in READ_AS:
        s = re.sub(pat, rep, s)
    s = re.sub(r"(?<![0-9.])\d{5,}(?![0-9.])", _man, s)
    s = COUNTER_RE.sub(_counter, s)
    s = re.sub(r"、\s*([、。])", r"\1", s)
