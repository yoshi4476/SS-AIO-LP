...
def stuck_floor():
    """止まっている記事の下限。いまは全記事と同じ3本にしている。

    経緯: 12本 → 8本 → 3本。**どの数字にも裏づけが無かった。**
    実測では 1〜10位の被リンク中央値は5本で、11〜30位で止まっている記事の
    9〜10本より少ない。`validate_rules` でも内部リンクの本数は成果を分けておらず、
    順序が入れ替わる。効きの確認できない下限を厚く守った結果、
    1記事に36本のリンク段落が積み上がっても1本も外せなくなっていた。

    `guarantee.py` が使う基準（3本以上）に揃える。厚く守る理由が出てきたら、
    そのときは実測を添えて戻す。
    """
    return {}


def fix(path, item, write, inb=None, floor=3, floors=None):
