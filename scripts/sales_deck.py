...
PRICE = {
    ...
    "init_2": "10万〜15万円",
    "run_1": "月額15万円〜", "run_2": "月額25万円", "run_3": "月額32万円",
    "articles": 60,
}
...
.flow li {{ counter-increment:f;position:relative;padding:11px 0 11px 46px;
  border-bottom:1px dotted {LINE};font-size:9.6pt;line-height:1.95; }}
.flow li:last-child {{ border-bottom:0; }}
.flow li::before {{ content:counter(f,decimal-leading-zero);position:absolute;left:0;top:12px;
  width:32px;height:32px;border-radius:9px;background:{NAVY};color:#fff;
  font-size:9pt;font-weight:bold;text-align:center;line-height:32px; }}
.flow b {{ display:block;font-size:10.5pt;color:{NAVY}; }}
...
<tr><td>本文</td><td>5,000字以上（一般的なSEO記事の1.5〜2倍の分量）</td></tr>
<tr><td>画像</td><td>アイキャッチ1枚＋本文の図解3〜5枚（すべてオリジナル作成）</td></tr>
<tr><td>FAQ</td><td>5問以上。構造化データと完全一致</td></tr>
<tr><td>リンク</td><td>関連記事への内部リンク3本以上＋出典となる外部リンク</td></tr>
<tr><td>根拠</td><td>出典付きの数値ファクト3箇所以上</td></tr>
<tr><td>導線</td><td>目次・問い合わせCTA 2箇所以上・関連記事・前後の記事ナビ</td></tr>
</table>
</div>
