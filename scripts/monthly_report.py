...
def fix_list(d):
    ...
    for page, ss in pages.items():
        ...
        # 5割前後の離脱はどのサイトでも起きるため、そこまで最優先にすると
        # 毎月すべてのページが最優先になり、本当に危ない月が埋もれる。
        if len(ss) >= 3:
            ...
            if p < 55:
                out.append((
                    "最優先" if p < 40 else "高",
                    f'<code>{page}</code> はファーストビューで離脱している',
                    f'先頭の次の区画 <code>{second["name"]}</code> への到達が{p}%。'
                    + ("6割以上が1画面目で離れています。" if p < 40
                       else "半数近くが1画面目で離れています。")
                    + '冒頭の見出しと最初のひと押しを、'
                      '何の会社かが1行で分かる文言に変えます'))
    if not sec:
        out.append((
...
def fetch_demo():
    ...
    return {
        ...
        "content": {"published": 8, "rows": [
            {"date": "2026/07/05", "title": "AIO対策とは？AI検索に引用される5つの手順", "score": "94", "status": "公開済み"},
            {"date": "2026/07/09", "title": "LLMO対策とは？ChatGPTに引用される7つの方法", "score": "96", "status": "公開済み"},
            {"date": "2026/07/14", "title": "MEO対策のやり方7ステップ", "score": "92", "status": "公開済み"},
            {"date": "2026/07/19", "title": "Googleマップの口コミを増やす方法5選", "score": "95", "status": "公開済み"},
            {"date": "2026/07/27", "title": "AI集客の完全ガイド", "score": "97", "status": "公開済み"},
        ]},
        "pages": [
...
{"".join(_ib_row(r) for r in ib["rows"][i:i + IB_PER_PAGE])}</table>
    ...
    glossary = [
        ...
        ("品質スコア", "当社の3観点採点（一次性・抽出性・決定支援／各100点）。各80点以上かつ総合90点以上のみ公開"),
...
<tr><td><b>3観点の採点</b></td><td>各100点・総合は平均</td>
