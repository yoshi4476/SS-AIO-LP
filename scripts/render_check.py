...
# 症状 → (説明, 見つけ方, 直し方)
# 画像の src が空か制御文字。chr(1) などは目に見えないので、範囲で指定する
IMG_BROKEN = re.compile(r'<img[^>]*src="([\x00-\x1f]*)"')
# 生成物に紛れた制御文字（改行・タブは除く）
...
SYMPTOMS = [
    ...
    ("押せないリンクがある", lambda t, x: re.search(r"<a[^>]*href=[\"']{2}", x),
     "リンク先が空。原稿のURLを確認する"),
    # 画像が出ない壊れ。実測で376個・134ページが src=制御文字 のまま配信されていた。
    # 原因は build.py の置換で、後方参照が制御文字として埋め込まれたこと。
    # 「空srcを探す」検査では見つからない（src="" ではなかった）
    ("画像が表示されない",
     lambda t, x: re.search(IMG_BROKEN, x),
     "src が空か制御文字。build.py の置換を確認する（原稿は正しいことが多い）"),
    # 置換処理でエスケープが壊れると、制御文字がそのまま生成物に出る
    ("制御文字が混ざっている",
     lambda t, x: re.search(CTRL, x),
     "生成物に制御文字。置換処理でエスケープが壊れていないか確認する"),
